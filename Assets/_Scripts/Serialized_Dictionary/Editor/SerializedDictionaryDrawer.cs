...
namespace LTF.SerializedDictionary.Editor
{
    ...
    public class SerializedDictionaryDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            bool expanded = _entriesProperty.isExpanded
        ...
        private void OnEnable(SerializedProperty property)
        {
            ...
            _reorderableList = new(property.serializedObject, _entriesProperty, !_isReadOnly, true, !_isReadOnly, !_isReadOnly)
            {
                ...
                {
                    float keyHeight = EditorGUI.GetPropertyHeight(_entriesProperty
                        .GetArrayElementAtIndex(index)
                        .FindPropertyRelative("Key"));
                    float valueHeight = EditorGUI.GetPropertyHeight(_entriesProperty
                        .GetArrayElementAtIndex(index)
                        .FindPropertyRelative("Value"));
                    float height = 8f + Math.Max(keyHeight, valueHeight);
                ...
                drawElementCallback = (rect, index, istActive, isFocused) =>
                {
                    ...
                    float halfSizeX = rect.size.x * .5f;
                    SerializedProperty keyProp = _entriesProperty.GetArrayElementAtIndex(index).FindPropertyRelative("Key");
                    const float LeftOffset = 100f;
                    Vector2 sizeKey = new(halfSizeX - LeftOffset, rect.size.y);
                    EditorGUI.PropertyField(new(rect.position, sizeKey), keyProp, GUIContent(""/*keyProp.type*/), true);

                    SerializedProperty valueProp = _entriesProperty.GetArrayElementAtIndex(index).FindPropertyRelative("Value");
                    const float RightOffset = 58f;
                    Vector2 sizeValue = new(halfSizeX + RightOffset, rect.size.y);
                    Vector2 positionValue = rect.position + new Vector2(sizeKey.x + 25f, 0f);
